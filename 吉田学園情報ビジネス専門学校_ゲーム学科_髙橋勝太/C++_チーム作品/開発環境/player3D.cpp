	...
	if (bUse == false)
	{
		...
		if (pInputKeyboard->GetPress(DIK_W) == true ||pGamepad->IsButtonPush(CInput::UP) == true)
		{
			...
			if (pInputKeyboard->GetPress(DIK_A) == true || pGamepad->IsButtonPush(CInput::LEFT) == true)
			{
				...
				//移動量計算
				m_move.x -= cosf(m_rot.y - D3DX_PI / 4) * PLAYER_SPEED;
				m_move.z += sinf(m_rot.y - D3DX_PI / 4) * PLAYER_SPEED;
			...
			{
				// 下方向
				m_move.x -= sinf(m_rot.y) * PLAYER_SPEED_BACK;
				m_move.z -= cosf(m_rot.y) * PLAYER_SPEED_BACK;
			}
		}
		else
		{
