...
void CPlayer3D::Update(void)
{
	...
	if (m_nCnt == 150)
	{
		m_pos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);		//プレイヤーの初期位置に戻す
	...
	if (bUse == false)
	{
		...
		if (pInputKeyboard->GetPress(DIK_S) == true || pGamepad->IsButtonPush(CInput::DOWN) == true)
		{
			...
			if (pInputKeyboard->GetPress(DIK_A) == true || pGamepad->IsButtonPush(CInput::LEFT) == true)
			{
				...
				//移動量計算
				m_move.x -= cosf(RotCameraY + D3DX_PI / 4) * (float)PLAYER_SPEED;
				m_move.z += sinf(RotCameraY + D3DX_PI / 4) * (float)PLAYER_SPEED;
			}
		...
		if (pInputKeyboard->GetPress(DIK_W) == true ||pGamepad->IsButtonPush(CInput::UP) == true)
		{
			...
			if (pInputKeyboard->GetPress(DIK_A) == true || pGamepad->IsButtonPush(CInput::LEFT) == true)
			{
				...
				//移動量計算
				m_move.x -= cosf(m_rot.y - D3DX_PI / 4) * (float)PLAYER_SPEED;
				m_move.z += sinf(m_rot.y - D3DX_PI / 4) * (float)PLAYER_SPEED;
			...
			{
				// 下方向
				m_move.x -= sinf(m_rot.y) * (float)PLAYER_SPEED_BACK;
				m_move.z -= cosf(m_rot.y) * (float)PLAYER_SPEED_BACK;
			}
		}
		else
		{
