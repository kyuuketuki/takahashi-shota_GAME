...
{
	//変数初期化
	m_fCollisionSize = 10;			//当たり判定の大きさ
	m_bMotionChange = false;		//モーショ状態の初期化
	m_nBrendFrame = 5;				//ブレンドフレームの初期化
}
...
void CPlayer3D::Update(void)
{
	...
	if (CGame::GetSwitchCollision() == false)
	{
		...
		for (int nCount = 0; nCount < n1 + n2; nCount++)
		{
			...
			{
				//オブジェクト3Dの当たり判定
				pObj->CollisionVec(&m_pos, &m_posold, &m_move, m_MaxPlayer.x - m_fCollisionSize, m_MinPlayer.x + m_fCollisionSize, m_MaxPlayer.z - m_fCollisionSize, m_MinPlayer.z + m_fCollisionSize, m_MaxPlayer.y, m_MinPlayer.y);
			}
		}
		...
		//頂点オブジェクトの当たり判定
		pVer->CollisionVec(&m_pos, &m_posold, &m_move, m_MaxPlayer.x - m_fCollisionSize, m_MinPlayer.x + m_fCollisionSize, m_MaxPlayer.z - m_fCollisionSize, m_MinPlayer.z + m_fCollisionSize, m_MaxPlayer.y, m_MinPlayer.y);
	}
