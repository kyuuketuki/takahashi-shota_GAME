...
//グローバル変数宣言
D3DXVECTOR3		g_posObjectVector[256][4];				//テキストデータの位置情報
int				g_nCntVertex[256];						//生成番号記憶
...
bool			g_bObjectVector[256];					//テキストデータの可視化オンオフ状態
...
{
	//モデルを生成
	CVertex *pObject3D = NULL;
	pObject3D = new CVertex;

	//位置情報を記憶
	g_posObjectVector[g_FailCntVecter][0] = fMaxSizeX;
	g_posObjectVector[g_FailCntVecter][1] = fMinSizeX;
	g_posObjectVector[g_FailCntVecter][2] = fMaxSizeY;
	g_posObjectVector[g_FailCntVecter][3] = fMinSizeY;
	
	//生成番号を記憶
	g_nCntVertex[g_FailCntVecter] = g_FailCntVecter;
